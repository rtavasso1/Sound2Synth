...
from utils.myLRScheduler import LinearWarmupCosineAnnealingLR
import torch
...
class Sound2SynthModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = LinearWarmupCosineAnnealingLR(
            optimizer,
            warmup_epochs=100,  # self.args.warmup_epochs,
            max_epochs=5000,  # self.args.num_epochs,
    ...
    def run_batch(self, batch, split="train", batch_idx=-1):
        (specs, sample_rates), (parameters, gradients) = batch

        pred = self(specs)
        parameters = [self.interface.from_tensor(par) for par in parameters]
        gradients = [
            LoadJson(g) if g != "" else {key: 1 for key in self.interface.parameters()}
            for g in gradients
        ]
        weights = {
            key: torch.tensor([g[key] for g in gradients], dtype=torch.float).type_as(
                pred
            )
            for key in self.interface.parameters()
        }
        true = torch.stack([p.to_tensor() for p in parameters]).type_as(pred).detach()
        w = {k: w.detach() for k, w in weights.items()}
        audioloss = (
            self.criteria(pred, true, w, loss_type="audioloss")
            if self.args.dataset_type == "my_multimodal"
            else self.criteria(pred, specs['audio'], w, loss_type="audioloss", from_params=False, orig_len=specs['orig_len'])
        )
        self.last_loss = audioloss
        # celoss = self.criteria(pred, true, w)
        celoss = torch.tensor(0.0)

        result = {
            "src": batch,
            "prd": pred,
            "tgt": true,
            "loss": audioloss if self.args.audio_loss else celoss,
            "stats": {
                ...
                self.criteria.default_loss_type: float(celoss.item()),
                "audioloss": float(audioloss.item()),
            },
        }
        if split != "train":
        ...
        for stat in result["stats"]:
            self.log(
                ...
                result["stats"][stat],
                sync_dist=True,
                prog_bar=(stat == self.criteria.default_loss_type),
                on_epoch=True,
                logger=True,
            )
        return result
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        if batch_idx * self.args.batch_size < self.args.examples:
            ...
            for i, (prd, tgt) in enumerate(zip(result["prd"], result["tgt"])):
                ...
                SaveJson(
                    preset_prd, pjoin(inf_epoch, "%03d_pd.json" % instance), indent=4
                )
                SaveJson(
                    preset_tgt, pjoin(inf_epoch, "%03d_gt.json" % instance), indent=4
                )
...
def Identifier(args):
    return (
        f"{args.backbone}_{args.model}_{FORMATTED_TIME('%Y-%m-%d_%H.%M')}"
